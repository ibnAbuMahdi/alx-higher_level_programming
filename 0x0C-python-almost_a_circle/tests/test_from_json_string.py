...
class TestFromJSONBase(TestCase):
    ...
    def test_None(self):
        ...
        for inp in ins:
            self.assertEqual(R.from_json_string(B.to_json_string(inp)), [])

    def test_not_None(self):
        dcts = [{'x': 1, 'width': 4}, {'y': 8, 'height': 3}]
        dcts_s = '[{"x": 1, "width": 4}, {"y": 8, "height": 3}]'
        json_dicts = B.from_json_string(dcts_s)
        for dct, dctj in zip(dcts, json_dicts):
            self.assertDictEqual(dct, dctj)

        dcts = [{'x': 1, 'width': 4}, {'y': 8, 'height': 3}]
        dcts_s = '[{"x": 1, "width": 4}, "not a dict!", {"y": 8, "height": 3}]'
        json_dicts = B.from_json_string(dcts_s)
        for dct, dctj in zip(dcts, json_dicts):
            self.assertDictEqual(dct, dctj)
